...
5. **收官：** 三场景串测 + 架构图 + 进入第3月（Store/audit/baseline）。
...
### 第 3 月方向（已开课：Store / reindex / audit / baseline）

整月教材：[`MONTH3_DAY1-30_COMBINED.md`](./MONTH3_DAY1-30_COMBINED.md) · 入口 [`docs/MONTH3.md`](../MONTH3.md)

四周**一并覆盖**（不必再从旧四选一菜单里「只留一条」）：

| 周 | 方向 | 你会练到 |
|---|---|---|
| 1～2 | 检索可运维 | Store 抽象、reindex、可选 Pg |
| 2～3 | 工作流可追溯 | 审计、EDIT、多节点加深 |
| 3～4 | 评测可证明 | Eval 落盘、baseline、CI 门禁 |

OCR / 多模态留作兴趣向或更远期。连贯总图见 [CURRICULUM_CONTINUITY.md](./CURRICULUM_CONTINUITY.md)。
...
| 第3月能力 | 首周应能… |
|---|---|
| 检索可运维 | 接 Store + reindex API |
| 工作流审计 | Flow 历史 + 多节点 |
| 评测平台 | EvalRun 落盘 + baseline diff |

按合订讲义顺序推进即可；编码仍可「每周深挖一条」，但课程主线已固定为第3月整月。
...
| 现象 | 原因 | 处理 |
|---|---|---|
| 想同时深挖 OCR | 冲淡主线 | OCR 留兴趣向；主线跟 MONTH3 合订 |
| 跳过 baseline | 第4月无法证明没改坏 | D30 前至少留一次绿 run |
