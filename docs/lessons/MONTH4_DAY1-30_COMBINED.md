...
5. **收官：** 三场景串测 + 架构终图 + 作品集升级 + 进入第5月（受控写入）。
...
**Step 4｜列「明确不做」（10 分钟）**  
无登录页、无「跳过审批一键过账」按钮、无自动改模。  
若你同时在学 Web 轨道：本月 `console.html` 与 `erp-ai-console/` 可并存，规则见 [CURRICULUM_CONTINUITY.md](./CURRICULUM_CONTINUITY.md) §5。
...
## M4-D30 收官；进入第5月（受控写入）

### 为什么
第4月终点是 **可隔离、可反馈、可演示** 的学习版 ERP AI 助手。今天清点成果、锁定边界、确认第5月入口。
...
**已排定：** 第5月后进入第6月 Port/Adapter → 第7月规则引擎 → 第8月工作流可视化。  
观测平台化 / 真 pg 运维 / 灰度等留作第9月及以后可选，见 [CURRICULUM_CONTINUITY.md](./CURRICULUM_CONTINUITY.md)。
...
|---|---|
| 三场景彩排不过 | 以 D24～26 回修 |
| 第5月想并行观测+规则 | 做不深 | 按已开课主线；可选见 CONTINUITY |
| 删除失败记录 | 保留体现成长 |
...
**Q: 第5月学什么？**  
A: 已开课主线是**受控写入**（假账本 + WriteGateway + HITL）。见 [`MONTH5.md`](../MONTH5.md)。观测/规则/可视化等已排到后续月或第9月可选，见 [CURRICULUM_CONTINUITY.md](./CURRICULUM_CONTINUITY.md)。
