...
>
> **产品句（全文反复强调）：**
> **把 Flow 状态机画成可读图：节点、合法边、当前高亮、审计回放。可视化是理解与演示工具，不是放开 Agent 乱跳状态。APPROVE ≠ 生产过账；第5月起写入（若发生）仍经规则/Gateway→假账本。本月不接公司 BPM 平台。**
>
...
> **产品句：** 把 Flow 状态机画成可读图：节点、合法边、当前高亮、审计回放。可视化是理解与演示工具，不是放开 Agent 乱跳状态。APPROVE ≠ 生产过账；第5月起写入（若发生）仍经规则/Gateway→假账本。本月不接公司 BPM 平台。
...
<template>
  <aside class="hitl" aria-label="人工决策面板">
    <h3>人工审批</h3>
    <p class="warn">APPROVE ≠ 生产过账成功；写入结果见 APPLY_WRITE 节点与侧栏。</p>
...
3. APPLY_WRITE 转圈
4. 侧栏 WRITE_RESULT；说清 APPROVE≠生产过账
...
| 决策 | POST | /api/ai/flow/{id}/decide | APPROVE≠生产过账 |
