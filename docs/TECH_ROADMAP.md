...
**B 站视频：** 见 [BILIBILI.md](./BILIBILI.md)；合订本各 Day/章另有「建议视频」行。

**逐日/逐章前置：** `MONTH1～7` 合订本每个 Day、`MONTH8` 每章开头有一行  
`技术前置：此时应当学会 … 后再进行阅读`（对齐上表节点）。月入口 `MONTH*.md` 也标注本月技术节点。
...
| 2026-08-15 | MONTH1～8 教材补「技术前置」横幅，与本文节点对齐 |
| 2026-08-15 | 增加 BILIBILI.md，并在 MONTH1～8 各 Day/章补「建议视频」 |
